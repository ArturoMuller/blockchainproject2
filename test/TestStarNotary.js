...
it('lets user1 put up their star for sale', async() => {
    ...
    instance.putStarUpForSale(starId, starPrice, {from: user1});
    setTimeout(async function() {
        const actual = await instance.starsForSale.call(starId);
        assert.equal(actual, starPrice);
    }, 5000);
});
...
it('can add the star name and star symbol properly', async() => {
    let instance = await StarNotary.deployed();
    assert.equal(await instance.name(), "UdacityStar");
    assert.equal(await instance.symbol(), "UDC");
});
